...
public class PencilCaption extends InteractiveTextLine {
	...
	static {
		ImageIcon icon = (ImageIcon) Tracker.getResourceIcon("hand_cursor.gif", false); //$NON-NLS-1$
		handCursor = GUIUtils.createCustomCursor(icon.getImage(), new Point(5, 0), "Hand", Cursor.DEFAULT_CURSOR); //$NON-NLS-1$
	...
	public Interactive findInteractive(DrawingPanel panel, int xpix, int ypix) {
		if (!isEnabled() || !(panel instanceof TrackerPanel))
			return null;
		if ("".equals(getText().trim())) //$NON-NLS-1$
			return null;
		if (bounds == null)
			return null;
		TrackerPanel trackerPanel = (TrackerPanel) panel;
