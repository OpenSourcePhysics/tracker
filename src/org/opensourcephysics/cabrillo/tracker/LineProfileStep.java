...
//      trailingCorners[i] = new Corner();
	...
	/**
	 * Rotates this step about its center to keep it either horizontal or parallel
	 * to the x-axis.
	 */
	protected void rotate() {
		...
		if (Math.abs(theta) > .0000001) {
			...
			AffineTransform transform = AffineTransform.getRotateInstance(-theta, center.x, center.y);
	...
	private double[][] getTiltedProfileData(TrackerPanel trackerPanel) {
		...
		corners[0][0].x = lineEnd0.x - dx;
		corners[0][0].y = lineEnd0.y + dy;
		corners[0][1].x = lineEnd0.x + dx;
		corners[0][1].y = lineEnd0.y - dy;
		...
		corners[1][0].x = lineEnd1.x - dx;
		corners[1][0].y = lineEnd1.y + dy;
		corners[1][1].x = lineEnd1.x + dx;
		corners[1][1].y = lineEnd1.y - dy;
			...
			for (int i = 0; i < len; i++) {
				...
				if (!vertices.isEmpty()) {
					...
					while (it.hasNext()) {
						...
						if (column > 0)
							areas[column - 1][row] = quadAreas[1];
						if (row > 0) {
							areas[column][row - 1] = quadAreas[3];
							if (column > 0)
								areas[column - 1][row - 1] = quadAreas[2];
						}
					}
	...
	private void findGridSegments() {
		...
		for (int i = 0; i < 2; i++) {
			if (!Double.isNaN(endX[i].x))
				sorter.add(endX[i]);
		}
		for (int i = 0; i < sweepX[0].length; i++) {
			for (int j = 0; j < 2; j++) {
				if (!Double.isNaN(sweepX[j][i].x))
					sorter.add(sweepX[j][i]);
			}
		}
	...
	/**
	 * Fills the vertices array with xy grid vertices that lie inside the probe
	 * area.
	 */
	...
	private void findEndIntersections() {
		...
		for (int j = 0; j < 2; j++) { // j is end #
			...
			if (grid < larger) {
				...
				endX[j].setLocation(Double.NaN, Double.NaN); // indicates none found
			// y intersection
			...
			if (grid < larger) {
				...
				endY[j].setLocation(Double.NaN, Double.NaN); // indicates none found
			...
			if (prev == grid.segment.vertex) {
				...
				if (prev == grid.segment.lower) {
					if (grid.isVertical)
						...
						next = grid.higherX.y > grid.y ? grid.higherX : grid.lowerX;
					...
					next = grid == grid.segment.higher ? grid.segment.lower : grid.segment.higher;
			} else
				next = grid.segment.vertex;
		}
	...
	/**
	 * Finds the areas in each quadrant of the specified GridVertex. Skips areas
	...
	private double[] getAreas(GridVertex vertex, double[] knownValues) {
		...
		for (int i = 0; i < 4; i++) {
			...
			if (area == 0)
				knownValues[i] = getArea(vertex, i);
		}
		return knownValues;
	}
	...
	private double getArea(GridVertex vertex, int quadrant) {
		// quadrant is defined by the axis along which the "current" intersection is
		// chosen
	...
	class LineEnd extends TPoint {
		...
		/**
		 * Overrides TPoint setXY to allow only movement parallel to the x-axis, or
		 * horizontal if profile is alwaysHorizontal
		 *
		 * @param x the x coordinate
		 * @param y the y coordinate
		 */
	 ...
	 * 
	 * 1. A line profile generates data for regions that are 1 pixel apart along the
	 * line 2. Region dimensions are 1 pixel by (2s + 1) pixels where s is the
	 * spread 3. Regions are measured in a probe of width 1.0 pixel and length
	 * (2s+1) pixels 4. The probe has a position (image position of center) and tilt
	 * angle 5. The probe has a leading edge, trailing edge and two 1-pixel-long
	 * ends 6. Measured rgb values depend on overlapping areas between probe and
	 * image pixel grid 7. Every area is a polygon made of pixel grid lines and
	 * probe perimeter lines 8. Every point on a polygon is an Intersection of type:
	 * (a) intersection of a probe edge or end with the grid (GridIntersection) (b)
	 * grid vertex located inside the probe (GridVertex) (c) probe corner (Corner)
	 * 9. Every GridVertex consists of an x and y GridSegment. 10. Every GridSegment
	 * has a lower and higher Intersection. 11. Every polygon (except corner orphan)
	 * includes one or more grid vertices 12. Find pixel areas by stepping thru grid
	 * vertices and finding quadrant areas 13. Find quadrant areas by stepping ccw
	 * along linked intersections back to start 14. Ignore corner orphans for
	 * now--not significant since rgb are area-weighted
	 */
	...
	/**
	 * A class to hold grid segments. Each segment has an integer value and double
	 * min and max extents.
	 */
	...
	/**
	 * A class to hold grid vertices. Each vertext is two GridSegments that
	 * intersect.
	 */
