...
public class CircleFitterStep extends Step {
	...
	/**
	 * Sets the data point at a specified column and row. Replaces existing element,
	 * adds null elements if needed. Data is stored in array element
	 * DataPoints[col][row]
	 *
	 ...
	 */
	public void setDataPoint(DataPoint p, int column, int row, boolean refreshAndPostEdit,
			boolean reduceArrayLengthIfNull) {
		if (row < 0 || column < 0 || column >= dataPoints.length)
			return;
	 ...
	 * @param refreshAndPostEdit true to refresh circle, fire event and post undo
	 *                           edit
	 * 
	 */
	...
	public void removeDataPoint(DataPoint p, boolean postUndoableEdit, boolean fireEvents) {
		...
		circleFitter.invalidateData(Boolean.valueOf(fireEvents));
		if (fireEvents) {
			circleFitter.firePropertyChange(CircleFitter.PROPERTY_CIRCLEFITTER_DATAPOINT, null, circleFitter); //$NON-NLS-1$
		}
	...
	/**
	 * Gets the valid data points. A point is valid if non-null. This return points
	 * from all columns, with user-marked points (column 0) first.
	 */
	...
	protected Mark getMark(TrackerPanel trackerPanel) {
		...
		if (mark == null) {
			...
			for (int i = 0; i < dataCount; i++) {
				...
				if (selection == next)
					p = screenPoints[i + points.length];
			}
	...
	/**
	 * Returns true if the circle is valid (ie if at least 3 data points have been
	 * successfuly fit).
	 * 
	...
	public boolean isValidCircle() {
		int dataCount = getValidDataPoints().size();
		return dataCount > 2 && !Double.isInfinite(radius) && radius > 0 && radius < CircleFitterFootprint.MAX_RADIUS;
	}
...
//  	}
	...
	/**
	 * Refreshes the circle center and radius based on 4 or more points. Uses the
	 * Modified Least Squares method in closed form described in Dale Umbach & Kerry
	 * N. Jones, A Few Methods for Fitting Circles to Data, IEEE TRANSACTIONS ON
	 * INSTRUMENTATION AND MEASUREMENT, date unknown (2000?)
	 */
	...
	public void copy(CircleFitterStep step) {
		// copy only marked data points--attached points set in refreshAttachments()
		// method
	...
	// ______________________ inner DataPoint and CenterPoint classes
	...
	class DataPoint extends TPoint {
      ...
      if (doRefresh) circleFitter.refreshFields(n);
		...
		public void setScreenPosition(int x, int y, VideoPanel vidPanel, InputEvent e) {
			if (this.isAttached())
				return; // don't drag or nudge when attached to another point
