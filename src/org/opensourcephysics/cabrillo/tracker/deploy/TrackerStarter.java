...
public class TrackerStarter {
	...
	static String logFileName = "tracker_start.log"; //$NON-NLS-1$
...
//	static boolean qtPreferred = false;
	static String version = "4.8"; //$NON-NLS-1$
    ...
    // identify codeBaseDir
      ...
      if (OSPRuntime.isWindows()) {
	      userDocuments = WinRegistry.readString(WinRegistry.HKEY_CURRENT_USER, 
...
//		}
    ...
    boolean launched = true;
	    ...
	    launched = false;
			exceptions += er.getClass().getSimpleName()+": "+er.getMessage()+newline; //$NON-NLS-1$
		}
  ...
  private static void exitGracefully(String jarPath) {
  	...
  	if (jarPath!=null) {
	  	JOptionPane.showMessageDialog(null, 
	  			...
	  			+jarPath+"."+newline+newline //$NON-NLS-1$
    			+"For more information see "+trackerHome+"/"+logFileName+newline //$NON-NLS-1$ //$NON-NLS-2$
  		...
  		if (trackerHome==null) {
  	    	...
  	    	JOptionPane.showMessageDialog(null, 
  	    			...
  	    			+codeBaseDir+newline+newline
  	    			+"For more information see "+logFileName+newline //$NON-NLS-1$
  	    			+"For trouble-shooting or to download the latest installer,"+newline //$NON-NLS-1$
  	    			+"please see www.cabrillo.edu/~dbrown/tracker/."+newline+newline //$NON-NLS-1$
  	    			+"Problems:"+newline+exceptions,  //$NON-NLS-1$
  	    			"TrackerStarter Vers "+version+": Error Starting Tracker",  //$NON-NLS-1$ //$NON-NLS-2$
...
//    	preferredEngine = prefsXMLControl.getString("video_engine"); //$NON-NLS-1$
    	...
    	if (prefsXMLControl.getPropertyNames().contains("memory_size")) { //$NON-NLS-1$
    		memorySize = prefsXMLControl.getInt("memory_size"); //$NON-NLS-1$
  			showDebugMessage("preferred memory size: "+memorySize+" MB"); //$NON-NLS-1$ //$NON-NLS-2$
    	}
    	if (!loaded)
...
//  	}
    ...
    // assemble message for debugging
    ...
    // prepare to execute the command
  	...
  	// set environment variable for new process
  	Map<String, String> env = builder.environment();
		env.put("TRACKER_PORT", String.valueOf(port)); //$NON-NLS-1$

		// write the tracker_start log
		showDebugMessage("set environment variable TRACKER_PORT: "+String.valueOf(port)); //$NON-NLS-1$
    writeLog();
...
//          process.waitFor();
//	        System.out.println("Tracker finished: exiting"); //$NON-NLS-1$
//          System.exit(0);
...
//  	new Thread(runner).start();
  ...
  private static void writeLog() {
    ...
    File file = new File(trackerHome, logFileName);
    if (userDocuments!=null && new File(userDocuments+"/Tracker").exists()) {    	 //$NON-NLS-1$
    	file = new File(userDocuments+"/Tracker", logFileName); //$NON-NLS-1$
    }
    try {
  ...
  private static void showDebugMessage(String message) {
  	...
  	if (debug) {
  		JOptionPane.showMessageDialog(null, message, "debug", JOptionPane.PLAIN_MESSAGE); //$NON-NLS-1$
  	}
  }
...
//		if (preferredEngine==null) { // should only happen the first time Tracker is used
//	  	String jrePath = preferredVM!=null? preferredVM: javaHome;
//		  ExtensionsManager manager = ExtensionsManager.getManager();
