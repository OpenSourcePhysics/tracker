	...
	public static String findFFMPegHome(String trackerHome, boolean writeToLog) throws Exception {
		...
		if (trackerHome!=null) {
			...
			if (f.exists() && f.isDirectory()) {
				...
				if (writeToLog) showDebugMessage("ffmpegHome found relative to TrackerHome: "+ffmpegHome); //$NON-NLS-1$
			}
		}
		...
		if (ffmpegHome==null) {
			...
			if (writeToLog) showDebugMessage("environment variable FFMPEG_HOME: " + ffmpegHome); //$NON-NLS-1$
			if (ffmpegHome!=null && !fileExists(ffmpegHome)) {
				ffmpegHome = null;
				if (writeToLog) showDebugMessage("FFMPEG_HOME directory no longer exists"); //$NON-NLS-1$
			}			
		}

		if (ffmpegHome==null)
			throw new NullPointerException("ffmpegHome not found"); //$NON-NLS-1$
		if (writeToLog) showDebugMessage("using ffmpegHome: " + ffmpegHome); //$NON-NLS-1$
		return ffmpegHome;
	}
...
//		}
		...
		if (ffmpegHome!=null && new File(ffmpegHome).exists()) {
			...
			showDebugMessage("setting FFMPEG_HOME = " + ffmpegHome); //$NON-NLS-1$
