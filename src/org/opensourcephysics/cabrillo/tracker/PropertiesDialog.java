...
public class PropertiesDialog extends JDialog {
	...
	protected ArrayList<String> trkProps = new ArrayList<String>(), trkValues = new ArrayList<String>();
	...
	private void createGUI() {
		...
		if (hasVid || clip.getVideoPath() != null) {
			...
			if (video != null) {
				...
				Dimension d = video.getImageSize(true); // original size without filters
				int w = d.width;
				int h = d.height;
...
//		    length += TrackerRes.getString("TActions.Dialog.AboutVideo.Seconds"); //$NON-NLS-1$
				...
				BitSet badFrames = TrackerIO.findBadVideoFrames(trackerPanel, TrackerIO.defaultBadFrameTolerance, false,
						false, false); // don't show dialog, just get bad frames
	...
	class VideoTableModel extends AbstractTableModel {
		...
		public Object getValueAt(int row, int col) {
			return col == 0 ? vidProps[row] : vidValues[row];
		}
