...
public class TMouseHandler implements InteractiveMouseHandler {
	...
	public void handleMouseAction(InteractivePanel panel, MouseEvent e) {
		...
		case InteractivePanel.MOUSE_PRESSED:
			...
			// object, selected track, frame number, and marking cursor saved during
			// the last move can all be stale. Refresh the marking state at the press
			// location so a touch can automark without requiring a hover first.
			selectedTrack = trackerPanel.getSelectedTrack();
			frameNumber = trackerPanel.getFrameNumber();
			iad = trackerPanel.getInteractive();
	...
	private void startLongPress(TrackerPanel trackerPanel, TPoint point, MouseEvent e) {
		 ...
		 *     handler.showLongPressPopup$org_opensourcephysics_cabrillo_tracker_TrackerPanel$I$I(
		 *         handler.longPressPanel, handler.longPressX, handler.longPressY);
		 *   }
		 * }, 2000);
		 * return;
		 */
	...
	private void showLongPressPopup(TrackerPanel trackerPanel, int x, int y) {
