 ...
 * @version 1.0
 */
public class AVPDiagnostics extends Diagnostics {
	...
	static { // added by W. Christian
		if (!OSPRuntime.isJS) {
			...
			try {
				URL url = AVPDiagnostics.class.getProtectionDomain().getCodeSource().getLocation();
...
//		}
	 ...
	 */
	public static void aboutAVP(String request) {
		requester = request;
		aboutAVP();
	}
...
//			}
	...
	public static String getXuggleVersion() {
		...
		if (status == 0) { // xuggle working correctly
			try {
				String name = "com.avpkit.core.Version"; //$NON-NLS-1$
