...
/**
 * A class to display videos using the AVP/Xuggle library. AVP in turn uses
 ...
 * 
 */
public class AVPVideo extends MovieVideo implements SmoothPlayable, IncrementallyLoadable {
...
//	private long keyTS1 = Long.MIN_VALUE;
	 ...
	 * @throws IOException
	 */
	AVPVideo(String fileName, XMLControl control) throws IOException {
		super(fileName, null, control);
		// create and open a container
		// set properties
		OSPLog.finest("AVP video loading " + path + " local?: " + isLocal); //$NON-NLS-1$ //$NON-NLS-2$
		if (isExport) {
...
//		stopFailDetection();
	...
	protected void finalizeLoading() throws IOException {
		...
		System.out.println(
				"AVPVideo found " + firstDisplayPacket + " incomplete out of " + packetCount + " total packets");
				...
				while (offset < size) {
					...
					if (!picture.isComplete()) {
						if (!haveImages)
							...
							System.out.println("!! Video picture was incomplete! dts=" + dts + " index=" + index);
						continue;
					}
				}
				if (dts == lastDTS)
...
//						+ packet.getFormattedTimeStamp() + " " + picture.getFormattedTimeStamp() + " " + picture.isComplete());
	...
	@Override
	public void dispose() {
		 System.out.println("AVPVideo.dispose");
		super.dispose();
		disposeXuggle();
	}
...
//______________________________  private methods _________________________
	...
	private String openContainer() {
		...
		if (isLocal) {
			...
			raf = null;
			System.out.println("!!Video path should be local!" + path);
		}
...
//		+ " " + packet.getTimeStamp() + " " + picture.getFormattedTimeStamp());
	...
	/**
	 * A class to save and load AVPVideo data.
	 */
	static public class Loader extends MovieVideo.Loader {
		...
		protected VideoAdapter createVideo(XMLControl control, String path) throws IOException {
			AVPVideo video = new AVPVideo(path, control);
			setVideo(path, video, MovieFactory.ENGINE_AVP);
			return video;
		}
	}
