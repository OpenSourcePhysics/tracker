...
import com.avpkit.core.*;
import com.avpkit.core.ICodec.ID;
import com.avpkit.core.video.*;

/**
 * A class to record videos using the AVP/Xuggle video engine.
 */
public class AVPVideoRecorder extends ScratchVideoRecorder {
...
//	private IRational timebase = IRational.make(1, 9000);	
   ...
   */
	...
	protected void saveScratch() throws IOException {
		...
		VideoFileFilter vidFilter = (VideoFileFilter)fileFilter;
		format.setOutputFormat(vidFilter.getContainerType(), null, null);
...
// Just checking to make sure this is OK.
  ...
  /**
   * Opens/initializes the output stream using a specified format.
   * 
   ...
   */
	...
	private boolean openStream(IContainerFormat format, ICodec codec, IPixelFormat.Type pixelType) 
		...
		if (outContainer.open(scratchFile.getAbsolutePath(), IContainer.Type.WRITE, format)<0) {
			OSPLog.finer("AVP could not open output file"); //$NON-NLS-1$
			return false;
		}	
...
//			OSPLog.finer("AVP could not set compliance mode to experimental"); //$NON-NLS-1$
//			return false;
//    }

  	if (outStreamCoder.open()<0) {
			OSPLog.finer("AVP could not open stream encoder"); //$NON-NLS-1$
			return false;
  	}
	
		if (outContainer.writeHeader()<0) {
			OSPLog.finer("AVP could not write file header"); //$NON-NLS-1$
			return false;
		}
		return true;
	}
...
//    	outContainer.delete();
  ...
  /**
   * Converts a bgr source image to a AVP picture.
   *
   ...
   */
	 ...
	 * it, including which codecs AVP can write (mux) into that container.
	 * 
	...
	public static void getSupportedCodecs(String name) {
	  ...
	  if (codecs.isEmpty())
	    System.out.println("AVPVideoRecorder unsupported codecs for "+name); //$NON-NLS-1$
	  else {
	    System.out.print("AVPVideoRecorder " + name+" ("+format+") supports following codecs:"); //$NON-NLS-1$ //$NON-NLS-2$
	    for(ID id : codecs) {
	      if (id != null) {
