...
/**
 * This is a Filter that produces fading ghost images of dark objects
...
public class DarkGhostFilter extends Filter {
  ...
  public void refresh() {
    super.refresh();
    fadeLabel.setText(MediaRes.getString("Filter.DarkGhost.Label.Fade"));           //$NON-NLS-1$
    fadeSlider.setToolTipText(MediaRes.getString("Filter.DarkGhost.ToolTip.Fade")); //$NON-NLS-1$
    boolean enabled = isEnabled();
    fadeLabel.setEnabled(enabled);
    fadeSlider.setEnabled(enabled);
    fadeField.setEnabled(enabled);
    if(inspector!=null) {
      inspector.setTitle(MediaRes.getString("Filter.DarkGhost.Title")); //$NON-NLS-1$
      inspector.pack();
    }
  }
  ...
  private void setOutputToGhost() {
    ...
    for(int i = 0; i<pixels.length; i++) {
      ...
      ghost = (int) (255-(1-fade)*(255-values[i])); // faded value of prev input
      if(ghost<v) {
  ...
  static class Loader implements XML.ObjectLoader {
    ...
    public void saveObject(XMLControl control, Object obj) {
      DarkGhostFilter filter = (DarkGhostFilter) obj;
